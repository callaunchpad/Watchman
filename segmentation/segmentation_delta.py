...
def form_room_delta(img_files, height, width):
	...
	while unseen and i < len(img_files):
		...
		for h, w in unseen.copy():
			# colors = frame[h][w]
			focus_area = [] #List of all pixels that we are interested in
			# Check if segmentation map within radius 5 (arbitrary!) labeled person (color #CC05FF)
			radius = 5
			for j in range(-radius,radius+1):
				for k in range(-radius,radius+1):
					if h+j >= 0 and w+k >= 0 and h+j < len(frame) and w+k < len(frame[0]):
						focus_area.append(frame[h+j][w+k])
			if not done:
				done = True
				print(focus_area)
			if all([color != [150, 5, 61] for color in focus_area]):
				img_arr[h][w] = img[h][w]
...
if __name__ == "__main__":
	...
	image_arr, depth_map = form_room_delta(img_files, args.height, args.width)
